...
public class ImageDownloader {
	 ...
	 */
	public void imageDownload(String url,ImageView mImageView,String path,Context mContext,OnImageDownload download){
		...
		Bitmap bitmap = getBitmapFromFile(mContext,imageName,path);
		//先从软引用中拿数�?
		...
		//文件中也没有，此时根据mImageView的tag，即url去判断该url对应的task是否已经在执行，如果在执行，本次操作不创建新的线程，否则创建新的线程�?
			...
			MyAsyncTask task = new MyAsyncTask(url, mImageView, path,mContext,download);
			if(mImageView != null){
//				Log.i(TAG, "执行MyAsyncTask --> " + ImageUtil.flag);
				ImageUtil.flag ++;
				task.execute();
				//将对应的url对应的任务存起来
				map.put(url, task);
	 ...
	 * @return
	 */
	private Bitmap getBitmapFromFile(Context mContext,String imageName,String path){
		Bitmap bitmap = null;
		if(imageName != null){
			File file = null;
			String real_path = "";
			try {
				if(ImageUtil.getInstance().hasSDCard()){
					...
					real_path = ImageUtil.getInstance().getPackagePath(mContext) + (path != null && path.startsWith("/") ? path : "/" + path);
				}
	 ...
	 * @return
	 */
	private boolean setBitmapToFile(String path,Context mContext,String imageName,Bitmap bitmap){
		File file = null;
		String real_path = "";
		try {
			if(ImageUtil.getInstance().hasSDCard()){
			...
			if(ImageUtil.getInstance().hasSDCard()){
				...
				fos = mContext.openFileOutput(imageName, Context.MODE_PRIVATE);
			}
	 ...
	 * @param imageName
	 */
	private void removeBitmapFromFile(String path,Context mContext,String imageName){
		File file = null;
		String real_path = "";
		try {
			if(ImageUtil.getInstance().hasSDCard()){
	...
	private class MyAsyncTask extends AsyncTask<String, Void, Bitmap>{
		...
		public MyAsyncTask(String url,ImageView mImageView,String path,Context mContext,OnImageDownload download){
